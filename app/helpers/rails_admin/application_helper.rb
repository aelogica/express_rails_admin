  ...
  module ApplicationHelper
    ...
    def navigation(nodes_stack, nodes, level = 0)
      nodes.collect do |node|
        ...
        active_class = "active" if is_active?(node)
        nav_icon = node.navigation_icon ? %(<i class="#{node.navigation_icon}"></i>).html_safe : ''
        li = content_tag :li, 'data-model' => model_param, class: active_class do
    ...
    def is_active?(node)
      node.abstract_model.model_name == @abstract_model.model_name
    end
